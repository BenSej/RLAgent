...
class TabQPolicy(QPolicy):
    ...
    def qvals(self, states):
        ...
        for state in states:
            ...
            qs = self.model[state[0], state[1], state[2], state[3]]
            returnList.append(qs)
        return returnList
    ...
    def td_step(self, state, action, reward, next_state, done):
        ...
        @param state: the cu`rrent state
        ...
        ogq = self.model[state[0], state[1], state[2], state[3], action]
        if done:
            target = reward
        else:
            qLeft = self.model[state[0], state[1], state[2], state[3]][0]
            qRight = self.model[state[0], state[1], state[2], state[3]][1]
            if qRight > qLeft:
                bestAction = 1
            target = reward + self.gamma * self.model[next_state[0], next_state[1], next_state[2], next_state[3], bestAction]
        self.model[state[0], state[1], state[2], state[3], action] = self.model[state[0], state[1], state[2], state[3],
                            action] + self.lr * (target - self.model[state[0], state[1], state[2], state[3], action])
...
if __name__ == '__main__':
    ...
    policy = TabQPolicy(env, buckets=(1, 5, 1, 5), actionsize=actionsize, lr=args.lr, gamma=args.gamma)
